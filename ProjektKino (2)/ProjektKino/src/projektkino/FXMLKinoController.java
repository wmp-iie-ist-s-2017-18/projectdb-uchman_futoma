...
public class FXMLKinoController implements Initializable {
    ...
    @FXML
    private Label LabetInfoTickets;
    @FXML
    ...
    public void fillComboBox() {
        ...
        try {
            ResultSet SoldTickets = connec.createStatement().executeQuery("{ call ile_biletow() }");
            ...
            while (SoldTickets.next()) {
                int id = SoldTickets.getInt(1);
                System.out.println("ilosc biletow=" + id);
                LbSoldTickets.setText(Integer.toString(id));
            }
    ...
    private void loadDataFromDatabase(ActionEvent event) {
        ...
        try {
            ...
            fillComboBox();
            //zapytanie i przechowanie wyniku w resultsecie
            //rs = conn.createStatement().executeUpdate("DELETE FROM seans WHERE dzien < date('now') & godzina < time('now');");
            ResultSet rsRezerwacje = conn.createStatement().executeQuery("SELECT id_rezerwacji, nazwisko, nr_kontaktowy, ilosc_biletow FROM rezerwacja");
            ResultSet rsSeans = conn.createStatement().executeQuery("SELECT id_seansu, dzien, godzina, ilosc_wolnych_miejsc FROM Seans");  //  ustawienie wolnych miejsc
            ResultSet rsFilm = conn.createStatement().executeQuery("SELECT id_filmu, tytul, gatunek, rezyser, czas_trwania FROM film");
            ResultSet rsSala = conn.createStatement().executeQuery("SELECT id_sali, nr_sali, ilosc_miejsc FROM sala");
    ...
    private void dodajRezerwacje(ActionEvent event) {
        ...
        if (validateFieldsRezerwacje()){
            try {
                ...
                int idfilmu;
                if (rsIdFilmu.next()){
                    idfilmu = rsIdFilmu.getInt("id_filmu");
                    System.out.println(idfilmu);
                }
                ResultSet rsIdSeansu = st.executeQuery("SELECT id_seansu FROM Seans WHERE id_filmu='" + rsIdFilmu.getInt("id_filmu") + "' AND dzien='" + dzien + "' AND godzina='" + godzina + "'");
                int idseansu;
                if(rsIdSeansu.next()){
                    idseansu = rsIdSeansu.getInt("id_seansu");
                ...
                if (rsWM.next()){
                    rsWM.getInt("ilosc_wolnych_miejsc");
                }
    ...
    private void dodajFilm(ActionEvent event) {
        ...
        if (validateFieldsFilm()){
            try {
                int rs = conn.createStatement().executeUpdate("INSERT INTO film(tytul,gatunek,rezyser,czas_trwania) VALUES ('" + textfieldTytul.getText() + "','" + textfieldGatunek.getText() + "','" + textfieldRezyser.getText() + "','" + textfieldCzasTrwania.getText() + "')");
                conn.close();
...
private void brakZaznaczenia(String tytul, String komunikat1, String komunikat2) {
        ...
        if (tabZamowienie.isSelected()) {
            ...
            if (index >= 0) {
                try {
                    ...
                    int idseansu;
                    if(rsIdR.next()){
                        idseansu = rsIdR.getInt("id_seansu");
                        System.out.println(idseansu);
                    }
                    ...
                    if(rsM.next()){
                        rsM.getInt("ilosc_wolnych_miejsc");
                    }
    ...
    private void dodajSeans(ActionEvent event) {
        ...
        if(validateFieldsSeans()){
            try {
                ...
                if(rsWolne.next()){
                    rsWolne.getString("ilosc_miejsc");
    ...
    private void fillRezerwacje(MouseEvent event) throws SQLException {
        ...
        if (tableRezerwacje.getSelectionModel().getSelectedItem()!=null){
            ...
            Statement st = conn.createStatement();
            ResultSet rsF = st.executeQuery("SELECT film.tytul FROM film,Seans,rezerwacja,sala WHERE rezerwacja.id_rezerwacji='"
            ...
            //System.out.println("TEST1"+rsF.getString("tytul"));
            if(rsF.next()){
    ...
    private void przyciskAktualizujSeans(ActionEvent event) throws SQLException {
        if (doUpdateSeans){
            ...
            if(rsIdSali.next()){
                rsIdSali.getInt("id_sali");
                rsIdSali.getInt("ilosc_miejsc");
            }
